...
function check_status(){
	if [ -f "${CADDY_DIR}/${CADDY_BIN}" ]; then
		...
		if [ -z "${CADDY_PID}" ]; then
			...
			NAIVE_DOMAIN="$(grep "443" "/etc/caddy/${CADDY_CONF}" | awk -F ' ' '{print $2,$3}')"
			...
			CADDY_INFO="
  ${GREEN_BACK}Domain: ${NAIVE_DOMAIN}${DEFAULT_COLOR}
...
function install_caddy() {
	...
	read -e -r -p "CF_API (e.g. API): " CONF_API
	[ -z "${CONF_API}" ] && { __error_msg "CF_API cannot be empty."; exit 1; }
	...
	if [ "${SYSTEM_OS}" == "RHEL" ]; then
		...
		yum install -y ca-certificates curl firewalld git lsof libcap unzip zip
		firewall-cmd --permanent --zone=public --add-port=22/tcp
		systemctl start firewalld
		firewall-cmd --reload
	else
		apt update -y
		apt install -y ca-certificates curl git lsof libcap2-bin zip
	    <<-EOF
			y
		EOF
	...
	"${GOBIN}/xcaddy" build --with "github.com/caddyserver/forwardproxy@caddy2=github.com/klzgrad/forwardproxy@naive" --with "github.com/caddy-dns/cloudflare" --with "github.com/mholt/caddy-l4" --with "github.com/mastercactapus/caddy2-proxyprotocol" --with "github.com/imgk/caddy-trojan"
    ...
    wget -P "/usr/share/caddy" "https://github.com/zjy7723/caddy2/releases/download/v1/myfile.zip"
    unzip /usr/share/caddy/myfile.zip -d /usr/share/caddy
