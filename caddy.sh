...
function check_status(){
	if [ -f "${CADDY_DIR}/${CADDY_BIN}" ]; then
		...
		if [ -z "${CADDY_PID}" ]; then
			...
			CF_DNS="$(grep "acme_dns" "/etc/caddy/${CADDY_CONF}" | awk -F ' ' '{print $3}' )"
			...
			CADDY_INFO="
  ${GREEN_BACK}CF_API: ${CF_DNS}${DEFAULT_COLOR}
...
function install_caddy() {
	...
	"${GOBIN}/xcaddy" build --with "github.com/caddyserver/forwardproxy@caddy2=github.com/klzgrad/forwardproxy@naive" --with "github.com/caddy-dns/cloudflare" --with "github.com/mholt/caddy-l4" --with "github.com/mastercactapus/caddy2-proxyprotocol" --with "github.com/mholt/caddy-webdav"
